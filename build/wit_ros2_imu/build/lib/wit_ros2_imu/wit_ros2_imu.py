...
class IMUDriverNode(Node):
    ...
    def driver_loop(self):
        ...
        try:
            ...
            print(e)
            self.get_logger().error("\033[31mSerial port opening failure\033[0m")
            if rclpy.ok():
                rclpy.shutdown()
            return
    ...
    def imu_data(self):
        ...
        self.publish_imu_tf(stamp, qua)

    def publish_imu_tf(self, stamp, quaternion):
        transform = TransformStamped()
        transform.header.stamp = stamp
        transform.header.frame_id = 'world'
        transform.child_frame_id = self.imu_msg.header.frame_id
